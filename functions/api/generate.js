...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    const debit = await env.DB.prepare("UPDATE accounts SET credits = credits - 1, updated_at = datetime('now') WHERE id = ? AND credits > 0").bind(auth.account.id).run();
    if (!debit.meta.changes) return json({ error: '额度不足，请先购买' }, 402);
    ...
    const raw = textResult.response || textResult.result?.response || '';
    ...
    await env.DB.prepare("UPDATE accounts SET credits = credits + 1, updated_at = datetime('now') WHERE id = ?").bind(auth.account.id).run();
