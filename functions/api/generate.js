...
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    const textResult = await env.AI.run('@cf/meta/llama-3.3-70b-instruct-fp8-fast', { messages: [{ role: 'system', content: '只返回严格 JSON。' }, { role: 'user', content: prompt }], temperature: 0.7, max_tokens: 1400 });
